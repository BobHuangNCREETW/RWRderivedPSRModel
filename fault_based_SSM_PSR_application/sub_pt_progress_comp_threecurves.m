...
%%%
%--- additional plot small and large M for all scaling
if(strcmp(region,'IR')==1)
  tar_LargeM=6.7;tar_SmallM=6.2;
elseif(strcmp(region,'SEU')==1)
  tar_LargeM=6.4;tar_SmallM=6;
...
elseif(strcmp(region,'global')==1);
  SoF_used=outnm(index_dash(4)+1:end);
  if(strcmp(SoF_used,'SS')==1&strcmp(Comptype,'dip')==1)
    tar_LargeM=6.8;tar_SmallM=6.3;
  elseif(strcmp(SoF_used,'SS')==1&strcmp(Comptype,'ZBSZ')==1)
    tar_LargeM=6.3;tar_SmallM=5.8;
  elseif(strcmp(SoF_used,'RV')==1&strcmp(Comptype,'dip')==1)
    tar_LargeM=7.1;tar_SmallM=6.5;
  elseif(strcmp(SoF_used,'RV')==1&strcmp(Comptype,'ZBSZ')==1)
    tar_LargeM=6.8;tar_SmallM=6.1;
  elseif(strcmp(SoF_used,'NML')==1&strcmp(Comptype,'dip')==1)
    tar_LargeM=6.9;tar_SmallM=6.3;
  elseif(strcmp(SoF_used,'NML')==1&strcmp(Comptype,'ZBSZ')==1)
    tar_LargeM=6.8;tar_SmallM=6.1;
  end
else
...
for i=1:length(Mw)
  [pre_S2_PSR1(i),pre_S2_PSR_add2sigmaal1(i),pre_S2_PSR_minus2sigmaal1(i),ASP_used1(i),ASP_addsigma1(i),ASP_minussigma1(i),W_used1(i),W_add2sigmaal1(i),W_minus2sigmaal1(i),A_used1(i),A_addsigma1(i),A_minussigma1(i),RWR_used1(i),NZbor_regional_mean_SSM_used1,MBP_RWReq1_1]=sub_makeComparecurve(Lon,Lat,Mw(i),dip1,ZBSZ1,Rake);
  [pre_S2_PSR2(i),pre_S2_PSR_add2sigmaal2(i),pre_S2_PSR_minus2sigmaal2(i),ASP_used2(i),ASP_addsigma2(i),ASP_minussigma2(i),W_used2(i),W_add2sigmaal2(i),W_minus2sigmaal2(i),A_used2(i),A_addsigma2(i),A_minussigma2(i),RWR_used2(i),NZbor_regional_mean_SSM_used2,MBP_RWReq1_2]=sub_makeComparecurve(Lon,Lat,Mw(i),dip2,ZBSZ2,Rake);
  [pre_S2_PSR3(i),pre_S2_PSR_add2sigmaal3(i),pre_S2_PSR_minus2sigmaal3(i),ASP_used3(i),ASP_addsigma3(i),ASP_minussigma3(i),W_used3(i),W_add2sigmaal3(i),W_minus2sigmaal3(i),A_used3(i),A_addsigma3(i),A_minussigma3(i),RWR_used3(i),NZbor_regional_mean_SSM_used3,MBP_RWReq1_3]=sub_makeComparecurve(Lon,Lat,Mw(i),dip3,ZBSZ3,Rake);
...
H1=subplot(3,3,1);
  ...
  elseif(strcmp(Comptype,'dip')==1)
    ...
    elseif(strcmp(region,'SEU')==1)
      set(HL3,'position',[po(1)+0.514,po(2)-0.027,po(3)*0.7,po(4)*0.7],'FontSize',10,'fontweight','bold');
    else
      set(HL3,'position',[po(1)+0.512,po(2)-0.027,po(3)*0.7,po(4)*0.7],'FontSize',11,'fontweight','bold');
    end
  end
...
%----
  xlabel('M','fontsize',13,'fontweight','bold');
  if(Rake==-999)
    ylabel('PSR','fontsize',13,'fontweight','bold');
...
H6=axes('position',[0.15,0.15,0.195,0.195]);
  ...
  if(Rake==-999)
    SoF_print='SoF all';
    ylabel('PSR (all SoF model)','fontsize',13,'fontweight','bold');
  elseif((Rake>=-180&Rake<-150)|(Rake>=-30&Rake<30)|(Rake>=150&Rake<=180))
    SoF_print='SS';
    ylabel('PSR (SoF model)','fontsize',13,'fontweight','bold');
  elseif(Rake>=30&Rake<150);
    SoF_print='RV';
    ylabel('PSR (SoF model)','fontsize',13,'fontweight','bold');
  elseif(Rake>=-150&Rake<-30)
  ...
  text(0.1,0.85,SoF_print,'fontsize',10,'fontweight','bold','backgroundcolor','white','edgecolor','none','margin',1);
labelfontsize=12;
